...
def plot_price_with_overlays(
    features: pd.DataFrame, overlays: list[tuple[str, str | pd.Series]] | None = None
) -> go.Figure:
    fig = go.Figure()
    fig.add_trace(
        go.Candlestick(
    ...
    for label, series in overlays or []:
        values = features[series] if isinstance(series, str) else series
        fig.add_trace(
            go.Scatter(
                x=features["date"],
                y=values,
...
def plot_rsi(features: pd.DataFrame, rsi_col: str, oversold: float, overbought: float) -> go.Figure:
    fig = go.Figure(
        data=go.Scatter(
            x=features["date"],
            y=features[rsi_col],
    ...
    fig.add_hrect(y0=oversold, y1=overbought, fillcolor="gray", opacity=0.1, line_width=0)
    fig.update_layout(margin=dict(l=0, r=0, t=30, b=20), yaxis=dict(range=[0, 100]))
    return fig
...
with st.sidebar:
    ...
    strategy_choice = st.selectbox("Strategy", list(STRATEGY_OPTIONS.keys()), index=0)
    ...
    if strategy_key == "sma":
        ...
        oversold = st.slider("Oversold", 5.0, 45.0, 30.0, 1.0)
        overbought = st.slider("Overbought", 55.0, 95.0, 70.0, 1.0)
        if oversold >= overbought:
            st.warning("Oversold must be less than overbought")
...
if strategy_key == "sma":
    ...
    enriched_features, signals = generate_rsi_mean_reversion_signals(
        filtered_features,
        window=rsi_window,
        oversold=oversold,
...
with chart_tab:
    if strategy_key == "sma":
        ...
        overlays = [(f"SMA {short_window}", short_col), (f"SMA {long_window}", long_col)]
        st.plotly_chart(
            plot_price_with_overlays(enriched_features, overlays), use_container_width=True
        )
    else:
        rsi_col = f"rsi_{rsi_window}"
        st.plotly_chart(plot_price_with_overlays(enriched_features), use_container_width=True)
        st.plotly_chart(
            plot_rsi(enriched_features, rsi_col, oversold=oversold, overbought=overbought),
